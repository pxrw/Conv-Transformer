...
from model.model_block import Attn2AttnBlock, up_sampling_by_convt, up_sampling_by_interpolate, MultiPatchAttention, BinCenterPred
...
class Decoder(nn.Module):
    def __init__(self, args, channel_list):
        ...
        self.aspp = MultiPatchAttention(self.channel_list[0])
        self.attn_32 = Attn2AttnBlock(
        ...
        # self.conv_16 = nn.Conv2d(self.channel_list[1] + embed_dims_list[0] // 2, 
        #                          self.channel_list[1] + embed_dims_list[0] // 2, kernel_size=3, padding=1)
        # self.conv_8 = nn.Conv2d(self.channel_list[2] + embed_dims_list[1] // 2,
        #                         self.channel_list[2] + embed_dims_list[1] // 2, kernel_size=3, padding=1)
        # self.conv_4 = nn.Conv2d(self.channel_list[3] + embed_dims_list[2] // 2,
        #                         self.channel_list[3] + embed_dims_list[2] // 2, kernel_size=3, padding=1)
...
class Attn2AttnDepth(nn.Module):
    def __init__(self, args):
        ...
        self.bin_pred = BinCenterPred(in_features=512, out_features=256,
                                      max_depth=self.args.max_depth, min_depth=self.args.min_depth)
        self.conv = nn.Conv2d(in_channels=64, out_channels=256, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        decoder_out[0] = self.conv(decoder_out[0])
        bin_centers = self.bin_pred(decoder_out[-1])
        # print('bin_center shape:', bin_centers.shape)
